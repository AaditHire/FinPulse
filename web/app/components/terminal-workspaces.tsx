...
function HealthWorkspace() {
  ...
  return <>
    ...
    <div className="health-grid"><article className="terminal-card capacity-card"><div className="terminal-card-head"><span>Supabase database</span><b>{loading ? "MEASURING" : health?.configured === false ? "LOCAL MODE" : `${percent.toFixed(1)}% USED`}</b></div><div className="capacity-gauge"><span style={{ width: `${percent}%` }}/></div><div className="capacity-labels"><b>{loading ? "Loading…" : formatBytes(used)}</b><span>of 500 MB</span></div><div className="thresholds"><span>60% WARN</span><span>70% PRUNE</span><span>80% REJECT</span></div></article>
      <article className="terminal-card"><div className="terminal-card-head"><span>Keep-alive</span><b>{loading ? "CHECKING" : health?.keepalive ? "OBSERVED" : "NO EVENT"}</b></div><div className="health-hero"><CircleGauge size={27}/><p><b>{loading ? "Loading telemetry" : health?.keepalive ? "Database reachable" : "Awaiting daily check"}</b><span>{loading ? "Contacting the public health endpoint…" : health?.keepalive ? `Last checked ${new Date(String((health.keepalive as Record<string, unknown>).checked_at)).toLocaleString()}` : "The dedicated workflow is independent from ingestion."}</span></p></div></article></div>
    <article className="terminal-card"><div className="terminal-card-head"><span>Provider registry</span><b>{loading ? "LOADING" : `${providers.length} CHECKS`}</b></div><div className="provider-table"><div><b>Provider</b><b>Status</b><b>Latency</b><b>Last success</b></div>{providers.map((provider) => <div key={String(provider.provider)}><strong>{String(provider.provider)}</strong><span className={`provider-status ${String(provider.status)}`}>{String(provider.status)}</span><span>{provider.latency_ms == null ? "—" : `${provider.latency_ms} ms`}</span><span>{provider.last_success_at ? new Date(String(provider.last_success_at)).toLocaleString() : "—"}</span></div>)}{!providers.length ? <div className="workspace-empty">{loading ? "Loading provider health…" : "No provider checks have been recorded yet."}</div> : null}</div></article>
  </>;
}
