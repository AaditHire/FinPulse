...
function HealthWorkspace() {
  ...
  return <>
    ...
    <ErrorNotice message={error}/>
    <div className="health-grid"><article className="terminal-card capacity-card"><div className="terminal-card-head"><span>Supabase database</span><b>{health?.configured === false ? "LOCAL MODE" : `${percent.toFixed(1)}% USED`}</b></div><div className="capacity-gauge"><span style={{ width: `${percent}%` }}/></div><div className="capacity-labels"><b>{formatBytes(used)}</b><span>of 500 MB</span></div><div className="thresholds"><span>60% WARN</span><span>70% PRUNE</span><span>80% REJECT</span></div></article>
      <article className="terminal-card"><div className="terminal-card-head"><span>Keep-alive</span><b>{health?.keepalive ? "OBSERVED" : "NO EVENT"}</b></div><div className="health-hero"><CircleGauge size={27}/><p><b>{health?.keepalive ? "Database reachable" : "Awaiting daily check"}</b><span>{health?.keepalive ? JSON.stringify(health.keepalive) : "The dedicated workflow is independent from ingestion."}</span></p></div></article></div>
    <article className="terminal-card"><div className="terminal-card-head"><span>Provider registry</span><b>{providers.length} CHECKS</b></div><div className="provider-table"><div><b>Provider</b><b>Status</b><b>Latency</b><b>Last success</b></div>{providers.map((provider) => <div key={String(provider.provider)}><strong>{String(provider.provider)}</strong><span className={`provider-status ${String(provider.status)}`}>{String(provider.status)}</span><span>{provider.latency_ms == null ? "—" : `${provider.latency_ms} ms`}</span><span>{provider.last_success_at ? new Date(String(provider.last_success_at)).toLocaleString() : "—"}</span></div>)}{!providers.length ? <div className="workspace-empty">Provider checks appear after authenticated ingestion runs.</div> : null}</div></article>
  </>;
}
